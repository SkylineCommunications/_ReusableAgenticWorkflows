...
**[T2-6] Storage access is encapsulated (no Tier-3 / consumer bypass)**

Strict data encapsulation is the heart of the 3-tier model: storage must only be touched through the repository/ApiHelper layer. Scan UI, GQI, UDAPI-controller, automation-script and other consumer code for **direct storage access that bypasses the abstraction**:
...
- `[WARNING]` per consumer/UI/controller location that accesses storage directly, bypassing the repository/ApiHelper layer. Sets the `No direct storage access from Tier 3` capability flag to false.
